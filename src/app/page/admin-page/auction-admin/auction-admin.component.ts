...
export class AuctionAdminComponent implements OnInit {
  ...
  minutes=10;
   Ms=2629800000; //1 Month= Ms Milisecons;
     ...
     ms=60000; //60*1000
     ss=1000; //60*1000
  constructor( private productservice: ProductService, private upload: UploadService,private fb: FormBuilder,private toast:ToastrService,
  ...
  ngOnInit() {
    ...
    this.productservice.getAucrion().subscribe(data=>{
      if(data!=null){
  ...
  timeNewAuction(x){
    ...
    if(this.subTime(timeNowDate,timeLetestDate)<=0){
      ...
      TimeNewEnd=this.miliseconsToDate((this.DateToMili(this.stringToDate(timeNowString))+this.ms*x));
    ...
    console.log(TimeLetest)
    console.log(TimeNewEnd);//63746042400000
    return TimeNewEnd;
  }
  DateToMili(x){
      let a =0;
      a= x.Year*this.Ys+(x.Month-1)*this.Ms+x.Date*this.Ds+x.Hours*this.hs+x.Minutes*this.ms+x.Seconds*this.ss;
      console.log(x)
      return a;
  }
  miliseconsToDate(x){
    ...
    let date={
     ...
     let mili=x;
      ...
      let seconds_left=mili_left/1000;
        ...
        console.log(mili)
       // seconds_left = seconds_left- date.Y*this.Ys/1000+miliNow;
  ...
  deleteAuction(id){
    this.productservice.deleteAucT(JSON.stringify(id)).subscribe(res=>{
      ...
      if(res.data[0].delete=="success"){
        this.toast.success('delete Auction product', 'delete success!!')
      }else{
        this.toast.error('delete Auction product', 'delete error!!')
      }
      this.productservice.update();
    })
  }
  ...
  Update(){
    ...
    this.productservice.upDateAuction(JSON.stringify(this.itemUp)).subscribe(res=>{
      console.log(res);
      if(res.data[0].delete=="success"){
        this.toast.success('Update Auction product', 'Update success!!')
      }
      else{
        this.toast.error('Update Auction product', 'Update error!!')
      }
    })
   ...
   addAuction(){
    ...
    this.productservice.createAuction(JSON.stringify(this.itemAuction)).subscribe(res=>{
      ...
      if(res.message=="Success"){
        this.toast.success('Add Auction product', 'Add success!!');
      }
      else{
        this.toast.error('Add Auction product', 'Add error!!')
      }
      this.productservice.update();
    })
  }
